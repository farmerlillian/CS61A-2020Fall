...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_x=make_city('',lat,lon)
    name_a=get_name(city_a)
    name_b=get_name(city_b)
...
def sprout_leaves(t, leaves):
    ...
    sprout_leaves_branches=[]
    if is_leaf(t):
        return tree(label(t),leave_branches)
    else:
        # sprout_leaves_branches = [sprout_leaves(b, leaves) for b in branches(t)]
        # return tree(label(t), sprout_leaves_branches)
        for b in branches(t):
            sprout_leaves_branches+=[sprout_leaves(b,leaves)]
        return tree(label(t), sprout_leaves_branches)
...
def riffle(deck):
    ...
    # return [deck[(k+1)%2*k//2+k%2*(len(deck)//2+(k-1)//2)]for k in range(len(deck))]
