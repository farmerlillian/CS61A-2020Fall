...
def balanced(m):
    ...
    # left_m=end(left(m))
    # right_m=end(right(m))
    # if is_planet(left_m) and is_planet(right_m):
    #     return True
    # else:
    #     weight_left,weight_right=total_weight(left_m),total_weight(right_m)
    #     # print('weight',weight_left,weight_right)
    #     torq_left,torq_right=length(left(m))*weight_left,length(right(m))*weight_right
    #     # print('torq',torq_left,torq_right)
    #     if torq_left!=torq_right:
    #         return False
    #     elif is_mobile(left_m) and (not balanced(left_m)) :
    #             return False
    #     elif is_mobile(right_m) and (not balanced(right_m)):
    #             return False
    #     return True
    if is_planet(m):
        return True
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        left_tree=totals_tree(l)
        right_tree=totals_tree(r)
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t)==find_value:
        ...
        leaves_branches=[replace_leaf(b,find_value,replace_value) for b in branches(t)]
...
def has_path(t, word):
    ...
    # elif len(string)==1:
    #     return True
    # def has_path_helper(path,string):
    #     if len(string)==0:
    #         return True
    #     if len(path)<len(string):
    #         return False
    #     elif path[0]==string[0]:
    #         return has_path_helper(path[1:],string[1:])
    #     else:
    #         return has_path_helper(path[1:],string[0:])
    # return has_path_helper(path[1:],string[1:])
    if len(word) == 1:
        if label(t) == word:
            return True
...
def quadratic(x, a, b, c):
    ...
    "*** YOUR CODE HERE ***"
    extreme=-b/(2*a)
    t1,t2=lower_bound(x),upper_bound(x)
    p0=a*extreme*extreme+b*extreme+c
    p1,p2=a*t1*t1+b*t1+c,a*t2*t2+b*t2+c
    if t1<extreme<t2:
        lower,upper=min(p0,p1,p2),max(p0,p1,p2)
    else:
